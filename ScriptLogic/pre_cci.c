# 1 "c:\\users\\vteje001c\\desktop\\projects\\script logics\\scriptlogics\\\\combined_ScriptLogics.c"
...
# 1 "c:\\users\\vteje001c\\desktop\\projects\\script logics\\scriptlogics\\\\combined_ScriptLogics.c" 2
...
# 2 "c:\\users\\vteje001c\\desktop\\projects\\script logics\\scriptlogics\\\\combined_ScriptLogics.c" 2
...
{
	lr_end_transaction(lr_eval_string("{MASTER_TRANSNAME}"), 2);
...
{
	if(Option == 0){  
		lr_set_debug_message(0, 1);
		lr_log_message("Current Log Level is %d", lr_get_debug_message());
		}

	else if(Option == 1){  
		lr_set_debug_message(1, 1);
		lr_log_message("Current Log Level is %d", lr_get_debug_message());
		}

	else if(Option == 2){  
		lr_set_debug_message(16 | 4, 1);
		lr_log_message("Current Log Level is %d", lr_get_debug_message());
		}
	
	else if(Option == 3){  
		lr_set_debug_message(16 | 4 | 2, 1 );
		lr_log_message("Current Log Level is %d", lr_get_debug_message());
	}

	else if(Option == 4){  
		lr_set_debug_message(16 | 4 | 2| 8, 1 );
		lr_log_message("Current Log Level is %d", lr_get_debug_message());
	}
...
SaveArrayToParamter(char *capturedString, char *seperator, char *outVarName)
{
	...
	token = (char *)strtok(capturedString, seperator);  
	if (!token){
...
SaveArrayToCustomString(char *startString, char *midString, char *endString, char *inputParam, char *outBody)
{
	...
	char *outBody1;     
	int orderCount = strlen(*&inputParam);  

	for (loopCount = 1; loopCount<=orderCount; loopCount++)  
	{
	...
	outBody1 = (char *)malloc(strlen(outString) * sizeof(char));  
...
Add_Ndays(int NumDays, char *OutPARAM)  
{
...
FindFirstLastDay()		
	...
	while(i!=1)
	{
		lr_save_datetime ("%d", 0 +(86400*count), "pDate5");
		i = atoi(lr_eval_string ("{pDate5}"));	
		count++;
	}


	lr_save_datetime ("%d-%m-%Y", 0 + 86400*(count-2), "LastdayCurrentMonth");
	lr_save_datetime ("%d", 0, "pDate4");   
	j = atoi(lr_eval_string ("{pDate4}"));
	lr_save_datetime ("%d-%m-%Y", 0 - 86400*(j - 1), "FirstDayCurrentMonth");
	lr_save_datetime ("%d-%m-%Y", 0 - (86400*j), "LastdayLastMonth");
...
# 3 "c:\\users\\vteje001c\\desktop\\projects\\script logics\\scriptlogics\\\\combined_ScriptLogics.c" 2
...
# 4 "c:\\users\\vteje001c\\desktop\\projects\\script logics\\scriptlogics\\\\combined_ScriptLogics.c" 2
...
{	
	...
	lr_save_datetime ("%d", 0, "pDate4");
	lr_save_datetime ("%m/01/%Y", 0 -(86400*(atoi(lr_eval_string ("{pDate4}"))+1)), "pDate5");
	lr_output_message ("First Day of Previous Month is %s", lr_eval_string ("{pDate5}"));

	lr_save_datetime ("%m/01/%Y", 0 +(86400*31), "pDate6");
	...
	lr_end_transaction(lr_eval_string("{TRANSNAME}"), 2);	
...
# 5 "c:\\users\\vteje001c\\desktop\\projects\\script logics\\scriptlogics\\\\combined_ScriptLogics.c" 2
...
# 6 "c:\\users\\vteje001c\\desktop\\projects\\script logics\\scriptlogics\\\\combined_ScriptLogics.c" 2
